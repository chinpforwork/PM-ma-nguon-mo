...
def process_dir(
    ...
    #lấy exam_id trước khi truyền vào evaluationconfig
    global ID_EXAM
    if omr_files and template:
        # Lấy file đầu tiên để quét IDExam
        sample_file = omr_files[0]
        in_omr = cv2.imread(str(sample_file), cv2.IMREAD_GRAYSCALE)
        in_omr = template.image_instance_ops.apply_preprocessors(
            str(sample_file), in_omr, template
        )
        if in_omr is not None:
            response_dict, _, _, _ = template.image_instance_ops.read_omr_response(
                template, image=in_omr, name=str(sample_file), save_dir=None
            )
            omr_response = get_concatenated_response(response_dict, template)
            ID_EXAM = omr_response.get("IDExam", "Unknown")
            print(f"🚀 IDExam lấy được trong process_dir: {ID_EXAM}")
            print(f"🚀 IDExam lấy được trong process_dir: {type(ID_EXAM)}")
