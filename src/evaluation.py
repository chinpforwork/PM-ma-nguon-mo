...
# so sánh câu trả lời của thí sinh với đáp án đúng và tính điểm
class AnswerMatcher:
    ...
    def validate_and_get_answer_type(self, answer_item):
        if (
                self.is_standard_answer(answer_item)
                and len(answer_item) == 4
                and all(char in ["D", "S"] for char in answer_item)
        ):
            return "TF-correct"
        elif self.is_standard_answer(answer_item):
            return "standard"
    ...
    # cập nhật cách chấm điểm của câu hỏi dựa trên loại đáp án
    ...
    def get_section_explanation(self):
        ...
        if answer_type in ["standard", "multiple-correct"]:
            ...
            return f"TF: {self.section_marking_scheme.section_key}"
    ...
    def get_verdict_marking(self, marked_answer):
        ...
        if answer_type == "standard":
            ...
            question_verdict = self.get_tf_correct_verdict(marked_answer)
        return question_verdict, self.marking.get(question_verdict,0)
    ...
    def get_tf_correct_verdict(self,marked_answer):
        ...
        correct_count = 0
        for i in range(4):
            if marked_answer[i] == allowed_answer[i] and marked_answer[i] in ["D", "S"]:
                correct_count += 1
        if correct_count > 0:
...
#quản lý cách chấm điểm của từng phần
class SectionMarkingScheme:
    ...
    def parse_scheme_marking(self, marking):
        # xử lý dạng TF
        if self.section_key.startswith("QTYPE_TF"):
            parsed_marking = {}
            for correct_count, scores in marking.items():
                parsed_marking[f"correct-{correct_count}"] = parse_float_or_fraction(scores["correct"])
            parsed_marking[f"incorrect"] = 0
            parsed_marking[f"unmarked"] = 0
...
#xử lý đầu vào,lưu thông tin bài kiểm tra và quy tắc chấm điểm
class EvaluationConfig:
    """Note: this instance will be reused for multiple omr sheets"""

    def __init__(self, curr_dir, evaluation_path, template, tuning_config,exam_id):
        ...
        if source_type == "csv":
            ...
            if os.path.exists(csv_path):
                ...
                self.exam_ids = answer_key.iloc[0, 1:].tolist()
                answer_key = answer_key.iloc[1:]
                answer_key.columns = ["question"] + self.exam_ids
                answer_key = answer_key.reset_index(drop=True)

                self.questions_in_order = answer_key["question"].to_list()
                # từng đáp án sẽ tương ứng với từng exam_id
                answers_in_order = answer_key[self.exam_ids].T.values.tolist()
        ...
        # ánh xạ câu hỏi với phần section trong marking_scheme
        ...
        for section_key, section_scheme in marking_schemes.items():
            ...
            #nếu khác thì thêm vào trong self.section_marking_schemes
            if section_key != DEFAULT_SECTION_KEY:
                ...
                for q in section_marking_scheme.questions:
                    ...
                    #thêm vào trong self.question_to_scheme
        ...
        #tạo ds answermatcher để chấm điểm từng câu
    ...
    def set_exam_id(self, exam_id):
        """Cập nhật exam_id cho bài thi hiện tại."""
        self.cur_exam_id = exam_id
    ...
    #chấm điểm một câu hỏi cụ thể
    def match_answer_for_question(self, current_score, question, marked_answer):
        if self.cur_exam_id not in self.exam_ids:
            logger.warning(
                f"Not exist exam_id: {self.cur_exam_id}"
            )
            return 0
        answer_matcher = self.question_to_answer_matcher[self.cur_exam_id][question]
    ...
    #kiểm tra số lượng câu hỏi và số lượng đáp án có = nhau k
    def validate_questions(self, answers_in_order):
        questions_in_order = self.questions_in_order
        len_questions_in_order = len(questions_in_order)

        # Kiểm tra độ dài của từng danh sách đáp án trong answers_in_order
        for exam_index, exam_id in enumerate(self.exam_ids):
            len_answers_for_exam = len(answers_in_order[exam_index])
            if len_questions_in_order != len_answers_for_exam:
                logger.critical(
                    f"For exam_id {exam_id}, questions_in_order({len_questions_in_order}): {questions_in_order}\n"
                    f"answers_in_order({len_answers_for_exam}): {answers_in_order[exam_index]}"
                )
                raise Exception(
                    f"Unequal lengths for questions_in_order and answers_in_order for exam_id {exam_id} "
                    f"({len_questions_in_order} != {len_answers_for_exam})"
                )

    #kieem tra có lỗi trong marking_scheme không trc khi chấm điểm
    ...
    #chuyển đổi ds đáp án thành các đối tượng answermatcher
    #đồng thời ánh xạ voiws tưng câu hỏi trong đề thi
    def parse_answers_and_map_questions(self, answers_in_order):
        question_to_answer_matcher = {}
        for exam_index, exam_id in enumerate(self.exam_ids):
            question_to_answer_matcher[exam_id] = {}
            for question_index, question in enumerate(self.questions_in_order):
                section_marking_scheme = self.get_marking_scheme_for_question(question)
                answer_item = answers_in_order[exam_index][question_index]  # Lấy đáp án cho exam_id hiện tại
                answer_matcher = AnswerMatcher(answer_item, section_marking_scheme)
                question_to_answer_matcher[exam_id][question] = answer_matcher
                if (
...
def evaluate_concatenated_response(
    ...
    exam_id = concatenated_response.get("IDExam", "Unknown")
    evaluation_config.set_exam_id(exam_id)
